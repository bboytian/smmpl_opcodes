...
def getSEA(latitude, longitude, when):
    date = when.timetuple()
    utc_offset = when.utcoffset().seconds//3600
...
def getAZ(latitude, longitude, when):
    date = when.timetuple()
    utc_offset = when.utcoffset().seconds//3600
...
def func(
    ...
    solhor = np.deg2rad(getSEA(lt, lg, timestamp))
    bearing = np.deg2rad(getAZ(lt, lg, timestamp))
